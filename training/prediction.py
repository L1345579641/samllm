...
def main():
    ...
    if not best_models:
        print("⚠️ 未在 MODEL_DIR 下找到任何 best_*_model.joblib 模型文件，请先运行 train.py 训练模型。")
        return

    # 🌟 精准锁定目标保存目录
    output_dir = Path(r"E:\吕工作\training_results\data")
    ...
    for model_path in best_models:
        ...
        try:
            ...
            print(f"❌ 无法加载模型 {model_path.name}，原因: {e}")
            continue
        ...
        # 🌟 核心内嵌：如果是千问14b，则精准提取对应的均值与不确定性标准差落盘
