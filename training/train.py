...
from config import DATA_DIR, EMBEDDING_DIR, LLM_MODEL_NAME, METRIC_DIR, MODEL_DIR, PREDICTION_DIR, TARGET_COLUMNS, \
    RANDOM_SEED, ensure_directories

# 🌟 导入自定义的工具（已彻底移除四大老算法类的导入） 🌟
...
def fit_scenario(name, features, y, train_index, test_index, seed, config, display_name):
    ...
    for model_name, estimator in models_dict.items():
        print(f"  -> 正在训练 {model_name:<16} ...", end="", flush=True)
        pipeline = Pipeline([
        ...
        # 🌟 核心优化：不仅打印平均 R2，还将 4 个具体的 R2 子指标一并列出 🌟
        r2_details = ", ".join([f"{rec['target'].split()[-1] if ' ' in rec['target'] else rec['target']}: {rec['r2']:.4f}" for rec in metric_records])
        print(f" 完成! (平均 R2: {summary['mean_r2']:.4f} | {r2_details})")

    best = max(results, key=lambda record: record["summary"]["mean_r2"])
    print(f"🏆 [{display_name}] 训练完毕，最轻装上阵模型: {best['model']} (R2: {best['summary']['mean_r2']:.4f})")
